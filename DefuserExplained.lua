-- defuser is a puzzel solver for the game defusal (https://www.roblox.com/games/3577061261/Defuse)

local defuser = {}

defuser.operations = {
	--[[
		these functions allow me to use operation variables within if statements without loadstring
		...
		e.g.
			concept:
				...
				print(x operation y) --> true

			execution:
				local operation = "<"
				local x, y = 1, 2

				print(defuser.operations[operation](x, y)) --> true
	]]
...
function defuser.interval(interval, number)
	--[[
		function that determines if a number is within an interval

		quick explanation of intervals

			"[]" are closed brackets and includes the number they're next to
			"()" are open and exlude the number they're next to
		...
		this function handles the intervals above but also another type
	...
	if interval:find("[><=]=?") then
		--[[
			checks if its the [operator][number] interval

				"[]" = either or, so "[><=]" looks for either >, <, or =
				"?" = isnt required, so =? checks if theres a "=" or not
				
				"[><=]=?" will find the following strings

					">", "<", "=", ">=", "<=", "==" (all of our operators excluding "=")

			if its that type of interval it then breaks it down and gets the operator and the number
			
			it works with rational numbers by checking if theres a decimal ("[.]") after the first set of numbers ("%d+") and then checks for numbers after the decimal ("%d*")

			it then uses the operator (interval[1]), the operation functions, the number arg, and the number within the interval (interval[2])
		]]
	...
	--[[
		if the interval doesnt fit the first format of [operator][number] then it assumes its a [bracket][number], [number][bracket]

		it breaks down the string getting the brakets ("[%[%(]" and "[%]%)]) and gets the rational numbers the same way it does above

		it then decides the operator based on the type of brackets

		after it returns if the number is within the interval
	]]
...
function defuser.list(dictionary)
	--[[
		simply turns a dictionary into a table
	]]
...
function defuser.strip(input, exclusions, length)
	--[[
		strips a string of every character not within the exclusions arg

		then checks if the stripped string is the desired length and returns the stripped string if so

		just realized the or nil part isnt actually needed
	]]
...
--[[
	the following functions are the solvers for puzzles and to truly understand their functionality you're gonna need to understand how to solve them

	the puzzel guide https://cdn.discordapp.com/attachments/606480627662979081/769147212742787082/Defusal_Manual.pdf?ex=66832f69&is=6681dde9&hm=c8cd1f20387f0dd53f6af110c69a6e0a7780c98238e84dcbdfbc8345da2ec510&
]]
...
function defuser.find(input)
	--[[
		looks through the defuser and skips the non puzzel members
		
		returns the found module or the string "invalid module"

		input doesnt have to be a 1:1 spelling

			e.g. defuser.find("hex") --> "hexadecimal"
	]]
...
function defuser.call(input, ...)
	--[[
		uses the find function to get the module and then checks if the puzzel exist
		
		if it does it then pcalls the function and either returns the result or the string "invalid input"

			this is to avoid having to do extra checks within the functions themselves (tldr: im lazy)
	]]
